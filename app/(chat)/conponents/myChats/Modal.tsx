...
import { BsArchive, BsArrowLeft, BsBoxArrowLeft, BsCheck, BsLock, BsMicMute } from "react-icons/bs";
...
const Modal = ({
  ...
  const items = [
    ...
    },
    {
      ...
      isHidden:chat?.isGroupChat? (chat?.groupAdmin || []).some(
  ...
  return (
    <div>
      <ul
        className={`z-[999999] absolute right-0 w-[250px] h-auto p-4  shadow-md   bg-gray-700 rounded duration-500 ring-2 ring-violet-500 ${
          open
      ...
      >
        {items.map((item, index) => (
          <li
            key={index}
            className={`flex items-center py-2 px-4 cursor-pointer   text-white hover:bg-gray-600 rounded duration-300 ${
