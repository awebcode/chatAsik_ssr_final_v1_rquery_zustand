...
const UserModal = ({ open, setOpen, isUserOnline }: any) => {
  ...
  const deleteSignleChatMutation = useDeleteSingleChatMutation(
    selectedChat?.chatId as any,true
  );
  const blockHandler = () => {
    const data = {
  ...
  return (
    <div
      className={`z-50 fixed max-h-screen max-w-3xl p-10 px-14 left-1/2 top-[100px] transform -translate-x-1/2 -translate-y-1/2 bg-gray-700 rounded duration-500 ring-2 ring-violet-500   ${
        open
          ? "translate-y-1 scale-100 opacity-100 duration-300"
          : "translate-y-0 scale-0 opacity-0 duration-300"
      }`}
    >
      <div className="flex flex-col gap-6 text-white mb-8">
